      ...
      <div className="w-full flex flex-row">
        <div className="w-full m-5 rounded-2xl shadow-xl break-all outline-dotted outline-1 outline-gray-500 pb-6 bg-gradient-to-r from-gray-300 to-gray-200">
                ...
                <div className="my-3 mx-6">
                    <textarea className="w-full resize-none rounded-lg p-2 focus:outline-none" id={"comment"} name={"comment"} rows="4" ref={commentRef}
                        cols="50" placeholder="Votre Avis ..." maxLength={1000}></textarea>
                </div>
        </div>
      </div>
      <button className="rounded-full bg-gray-100 outline-dotted outline-1 outline-gray-500 hover:outline-offset-2 w-10 h-10 absolute -mt-10 -ml-5 hover:bg-teal-500 hover:text-white" onClick={handleRating}>
        OK
      </button>
    </div>
    )
