  ...
  return (
    <div className="flex w-full h-18 fixed bg-gradient-to-r from-teal-500 to-amber-300 shadow-md z-50">
      ...
      <div>
        ...
        <button className={`w-8 text-white h-8 hover:outline-dashed hover:outline-1 hover:outline-gray-600 focus:outline-offset-2 hover:bg-gradient-to-t hover:from-amber-300 rounded-full m-2 ${
          (isVoiceOn)
            ? "outline-dashed outline-1 outline-gray-600 outline-offset-2 bg-gradient-to-t from-amber-300"
            : ""
        }`} onClick={handleSetIsVoiceOn}>{(isVoiceOn) ? 
        (
